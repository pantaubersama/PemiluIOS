...
class PenpolViewModel: ViewModelType {
    ...
    init(navigator: PenpolNavigator) {
        ...
        input = Input(searchTrigger: searchSubject.asObserver(), addTrigger: addSubject.asObserver(), filterTrigger: filterSubject.asObserver(), refreshTrigger: refreshSubject.asObserver())
        ...
        output = Output(searchSelected: search, filterSelected: filter, addSelected: add)
    }
}
